...
from typing import Iterator, Union, Union, Tuple
...
class file_part_mmap:
    def __init__(self, file_path, file_doclens, dim):
        ...
        mmap_storage = torch.HalfStorage.from_file(
            file_path, False, sum(self.doclens) * self.dim
        )
...
MMAP_LIST_ITEM = Union[file_part_mem, file_part_mmap]


class PartMMap(object):
    def __init__(self, index_path, part_doclens, dim, memtype="mmap") -> None:
        _, all_parts_paths, _ = get_parts(index_path)
        self.all_parts_paths = all_parts_paths
        self.part_doclens = part_doclens
        self.dim = dim
        if memtype not in ["mmap", "mem"]:
            assert False, "Unknown memtype %s" % memtype
        self.memtype = memtype

        if self.memtype == "mmap":
            self.all_parts_paths = [
                file.replace(".pt", ".store") for file in self.all_parts_paths
            ]

    def __getitem__(self, index: int) -> MMAP_LIST_ITEM:
        target_path, doclens = self.all_parts_paths[index], self.part_doclens[index]
        if self.memtype == "mmap":
            return file_part_mmap(target_path, doclens, self.dim)
        elif self.memtype == "mem":
            return file_part_mem(target_path, doclens, self.dim)

    def __iter__(self) -> Iterator[MMAP_LIST_ITEM]:
        for path, doclens in tqdm(
            zip(self.all_parts_paths, self.part_doclens),
            total=len(all_parts_paths),
            desc=f"Loading index shards with memtype '{self.memtype}'",
            unit="shard",
        ):
            if self.memtype == "mmap":
                yield file_part_mmap(path, doclens, self.dim)
            elif self.memtype == "mem":
                yield file_part_mem(path, doclens, self.dim)
            else:
                assert False, "Unknown memtype %s" % memtype
...
class re_ranker_mmap:
    def __init__(self, index_path, args, inference, verbose=False, memtype="mmap"):
        ...
        self.part_mmap: List[file_part_mmap] = re_ranker_mmap._load_parts(
            index_path, self.part_doclens, self.dim, memtype
        )
        ...
        # self.segment_sizes = torch.LongTensor(
        #     [0] + [x.mmap.shape[0] for x in self.part_mmap]
        # )
    ...
    def _load_parts(index_path, part_doclens, dim, memtype="mmap"):
        ...
        return PartMMap(index_path, part_doclens, dim, memtype=memtype)
        # _, all_parts_paths, _ = get_parts(index_path)
        ...
        #     ]
        #     mmaps = [
        #         file_part_mmap(path, doclens, dim)
        #         for path, doclens in zip(all_parts_paths, part_doclens)
        #     ]
        # elif memtype == "mem":
        #     mmaps = [
        #         file_part_mem(path, doclens, dim)
        #         for path, doclens in tqdm(
    ...
    def get_embeddings_copy_with_pids(self, pids, target):
        part_ids = [np.searchsorted(self.part_pid_end_offsets, pid) for pid in pids]
        part_id_groups = defaultdict(list)
        pid2index = {}
        for i, (part_id, pid) in enumerate(zip(part_ids, pids)):
            part_id_groups[part_id].append(pid)
            pid2index[pid] = i

        for part_id in part_id_groups:
            part_mmap = self.part_mmap[part_id]
            for pid in part_id_groups[part_id]:
                index = pid2index[pid]
                local_pid = pid - self.part_pid_begin_offsets[part_id]
                disk_tensor = part_mmap.get_embedding(local_pid)
                doclen = disk_tensor.shape[0]
                # only here is there a memory copy from the memory mapped file
                target[index, :doclen, :] = disk_tensor
        return target
    ...
    def our_rerank_with_embeddings(self, qembs, pids, weightsQ=None, gpu=True):
        ...
        D_ = torch.zeros(len(pids), self.doc_maxlen, self.dim)
        self.get_embeddings_copy_with_pids(pids, D_)
        # for offset, pid in enumerate(pid_iter):
        #     self.get_embedding_copy(pid, D_, offset)
        if gpu:
            D_ = D_.cuda()
    ...
    def our_rerank_with_embeddings_batched(
        self, qembs, pids, weightsQ=None, gpu=True, batch_size=10000
    ):
        # print(
        #     f"our_rerank_with_embeddings_batched: qembs ({len(qembs)}) pids ({len(pids)}), batch_size={batch_size}"
        # )
        import more_itertools
        ...
        allscores = []
        for group in tqdm(
            more_itertools.chunked(pids, batch_size),
            desc="our_rerank_with_embeddings_batched",
        ):
...
class ColBERTModelOnlyFactory:
    def __init__(
        ...
        if not gpu:
            ...
            colbert.parameters.DEVICE = (
                colbert.evaluation.load_model.DEVICE
            ) = colbert.modeling.colbert.DEVICE = torch.device("cpu")
            self.gpu = False
    ...
    def query_encoder(self, detach=True) -> pt.Transformer:
        ...
        def row_apply(df):
            if "docno" in df.columns or "docid" in df.columns:
                warn(
                    "You are query encoding an R dataframe, the query will be encoded for each row"
                )
        ...
        def df_apply(df):
            with torch.no_grad():
                ...
                for chunk in chunker(df, batch_size):
                    embsD, idsD = self.args.inference.docFromText(
                        chunk.text.tolist(), with_ids=True
                    )
                    if detach:
                        embsD = embsD.cpu()
    ...
    def _explain(self, query, embsD, idsD):
        ...
        ax1.bar(
            [0.5 + i for i in range(0, 32)],
            contributions,
            color=plt.cm.Blues(minmax_scale(contributions, feature_range=(0.4, 1))),
        )
        ax1.set_xlim([0, 32])
        ax1.set_xticklabels([])
        fig.tight_layout()
    ...
    def text_scorer(
        self, query_encoded=False, doc_attr="text", verbose=False
    ) -> pt.Transformer:
        """
        ...
        def _text_scorer(queries_and_docs):
            ...
            with torch.no_grad():
                for qid, group in (
                    tqdm(groupby, total=len(groupby), unit="q") if verbose else groupby
                ):
        ...
        def _text_scorer_qembs(queries_and_docs):
            ...
            with torch.no_grad():
                for qid, group in (
                    ...
                    query = group["query"].values[0]
                    ranking = slow_rerank_with_qembs(
                        self.args,
                        qembs,
                        group["docno"].values,
                        group[doc_attr].values.tolist(),
    ...
    def scorer(
        ...
        def _score_query(df):
            with torch.no_grad():
                ...
                D = torch.zeros(
                    len(df), factory.args.doc_maxlen, factory.args.dim, device=cuda0
                )
                ...
                if add_exact_match_contribution:
                    ...
                    idsD = torch.zeros(
                        len(df), factory.args.doc_maxlen, dtype=idsQ.dtype
                    )
                    ...
                    exact_match = (idsQ.unsqueeze(1) == idsD.unsqueeze(2)) & (
                        idsQ != 101
                    )
...
class ColBERTFactory(ColBERTModelOnlyFactory):
        ...
        if index_root is None or index_name is None:
            ...
            if os.path.exists(docnos_file):
                with pt.io.autoopen(docnos_file, "rb") as f:
                    ...
                    self.docno2docid = {
                        docno: docid for docid, docno in enumerate(self.docid2docno)
                    }
    ...
    def from_dataset(
        dataset: Union[str, Dataset], variant: str = None, version="latest", **kwargs
    ):
    ...
    def _faiss_index(self):
        ...
        self.faiss_index = FaissIndex(
            ...
            faiss_index_path,
            self.args.nprobe,
            self.args.part_range,
            mmap=self.faisstype == "mmap",
        )
        # ensure the faiss_index is transferred to GPU memory for speed
        import faiss

        if self.faiss_index_on_gpu:
            self.faiss_index.faiss_index = faiss.index_cpu_to_all_gpus(
    ...
    def set_retrieve(
        ...
        def _single_retrieve(queries_df):
            ...
            if "query_embs" in queries_df.columns:
                warn(
                    "set_retrieve() used with query_encoded=False, but query_embs column present in input. Should you use query_encoded=True?"
                )
            rtr = []
            ...
            rtrDf = pd.DataFrame(
                rtr, columns=["qid", "query", "docid", "query_toks", "query_embs"]
            )
            if docnos:
        ...
        return pt.apply.generic(
            _single_retrieve_qembs if query_encoded else _single_retrieve
        )
    ...
    def index_scorer(
        self, query_encoded=False, add_ranks=False, add_docnos=True, batch_size=10000
    ) -> pt.Transformer:
        """
        ...
        def rrm_scorer(qid_group):
            if "query_embs" in qid_group.columns:
                warn(
                    "index_scorer() used with query_encoded=False, but query_embs column present in input. Should you use query_encoded=True?"
                )
            ...
            if batch_size > 0:
                scores = rrm.our_rerank_batched(
                    qid_group.iloc[0]["query"],
                    docids,
                    batch_size=batch_size,
                    gpu=self.gpu,
                )
            else:
                scores = rrm.our_rerank(
                    qid_group.iloc[0]["query"], docids, gpu=self.gpu
                )
        ...
        def rrm_scorer_query_embs(qid_group):
            ...
            if batch_size > 0:
                ...
                scores = rrm.our_rerank_with_embeddings(
                    qid_group.iloc[0]["query_embs"], docids, weights, gpu=self.gpu
                )
            qid_group["score"] = scores
            if "docno" not in qid_group.columns and add_docnos:
                qid_group = self._add_docnos(qid_group)
    ...
    def get_embeddings_by_token(
        self, tokenid, flatten=True, sample=None
    ) -> Union[torch.TensorType, List[torch.TensorType]]:
        """
        ...
        def partition(
            tensor: torch.Tensor, offsets: torch.Tensor
        ) -> List[torch.Tensor]:
        ...
        if sample is not None:
            offsets = offsets[
                self.rng.integers(0, len(offsets), int(sample * len(offsets)))
            ]
        ...
        all_tensors = [
            rrm.part_mmap[shard].mmap[shard_portion - self.segment_starts[shard]]
            for shard, shard_portion in enumerate(partitioned_offsets)
            if shard_portion.shape[0] > 0
        ]
        ...
        def _single_retrieve(queries_df):
            ...
            for row in iter:
                ...
                if query_encoded:
                    ...
                    # NB: ids is 2D
                    qweights = (
                        row.query_weights.unsqueeze(0)
                        if weights_set
                        else torch.ones(ids.shape)
                    )
                else:
                    if "query_embs" in queries_df.columns:
                        warn(
                            "ann_retrieve_score() used with query_encoded=False, but query_embs column present in input. Should you use query_encoded=True?"
                        )
                    with torch.no_grad():
                        Q, ids, masks = self.args.inference.queryFromText(
                            [row.query], bsize=512, with_ids=True
                ...
                all_scores, all_embedding_ids = self._faiss_index().faiss_index.search(
                    Q_cpu_numpy, faiss_depth
                )
    ...
    def fetch_index_encodings(factory, verbose=False, ids=False) -> pt.Transformer:
        ...
        def _get_embs(df):
            rrm = (
                factory._rrm()
            )  # _rrm() instead of rrm because we need to check it has already been loaded.
            if verbose:
                import pyterrier as pt
    ...
    def prf(
        pytcolbert, rerank, fb_docs=3, fb_embs=10, beta=1.0, k=24
    ) -> pt.Transformer:
        """
        ...
        dense_e2e = pytcolbert.set_retrieve() >> pytcolbert.index_scorer(
            query_encoded=True, add_ranks=True, batch_size=10000
        )
        if rerank:
            prf_pipe = (
                dense_e2e
                ...
                >> (
                    pytcolbert.index_scorer(
                        query_encoded=True, add_ranks=True, batch_size=5000
                    )
                    % 1000
                )
            )
...
class ColbertPRF(pt.Transformer):
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            self.k > self.fb_embs
        ), "fb_embs should be smaller than number of clusters"
    ...
    def _init_clustering(self):
        ...
        if Version(sklearn.__version__) > Version("0.23.2"):
            warn(
                "You have sklearn version %s - sklearn KMeans clustering changed in 0.24, so performance may differ from those reported in the ICTIR 2021 paper, which used 0.23.2. "
                "See also https://github.com/scikit-learn/scikit-learn/issues/19990"
    ...
    def transform_query(self, topic_and_res: pd.DataFrame) -> pd.DataFrame:
        ...
        if "doc_embs" in topic_and_res.columns:
            ...
            prf_embs = torch.cat(
                [
                    self.pytcfactory.rrm.get_embedding(docid)
        ...
        for cluster, tok2freq in zip(range(self.k), toks2freqs):
            ...
            tid = self.fnt.inference.query_tokenizer.tok.convert_tokens_to_ids(
                most_likely_tok
            )
            emb_and_score.append(
                (centroids[cluster], most_likely_tok, tid, self.idfdict[tid])
            )
        ...
        newemb = torch.cat([first_row.query_embs, torch.Tensor(exp_embds)])

        # the weights column defines important of each query embedding
        weights = torch.cat(
            [torch.ones(len(first_row.query_embs)), self.beta * torch.Tensor(scores)]
        )

        # generate the revised query dataframe row
        rtr = pd.DataFrame(
            [[first_row.qid, first_row.query, newemb, toks, weights]],
    ...
    def transform(self, topics_and_docs: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in required:
            if not col in topics_and_docs.columns:
                raise KeyError(
                    "Input missing column %s, found %s"
                    % (col, str(list(topics_and_docs.columns)))
                )

        # restore the docid column if missing
        if (
            "docid" not in topics_and_docs.columns
            and "doc_embs" not in topics_and_docs.columns
        ):
...
def _approx_maxsim_numpy(faiss_scores, faiss_ids, mapping, weights, score_buffer):
    ...
    for rank in range(faiss_depth):
        rank_pids = pids[:, rank]
        score_buffer[rank_pids, qemb_ids] = np.maximum(
            score_buffer[rank_pids, qemb_ids], faiss_scores[:, rank]
        )
...
def _approx_maxsim_defaultdict(
    all_scores, all_embedding_ids, mapping, qweights, ignore2
):
    from collections import defaultdict
...
def _approx_maxsim_sparse(all_scores, all_embedding_ids, mapping, qweights, ignore2):
    ...
    for qpos in range(num_qembs):
        a = csr_matrix(
            (all_scores[qpos], (np.arange(faiss_depth), all_pids[qpos])),
            shape=(faiss_depth, index_size),
        )
